...
def get_papers_list() -> str:
    """
    List all papers stored in the system organized by topic.
    
    This resource provides an overview of all papers that have been searched
    and stored in the system, organized by research topic.
    """
    all_papers = {}
    
    # Traverse all topic directories
    if os.path.exists(PAPER_DIR):
        for topic_dir in os.listdir(PAPER_DIR):
            topic_path = os.path.join(PAPER_DIR, topic_dir)
            if os.path.isdir(topic_path):
    ...
    content = "# Stored Academic Papers\n\n"
    content += f"Total topics: {len(all_papers)}\n\n"
    
    for topic, info in all_papers.items():
        content += f"## {topic.replace('_', ' ').title()}\n"
        content += f"- Papers count: {info['count']}\n"
        content += f"- Paper IDs: {', '.join(info['papers'])}\n\n"
